...
const EditDigitalReleaseForm: React.FC<IEditDigitalReleaseProps> = ({
  ...
  const [availableFrom, setAvailableFrom] = useState(
    product.AvailableFrom.slice(0, 10),
  );
  const [availableTo, setAvailableTo] = useState(
    product.AvailableTo.slice(0, 10),
  );
  ...
  const handleSubmit = async (values: any) => {
    ...
    await onSubmit(
      ...
      product.CategoryId === selectedCategory.value ? false : true,
    );
  };
  ...
  useEffect(() => {
    if (
      // @ts-ignore
      categories.filter((category) => category.value === product.CategoryId)
        .length === 0
    )
      return;
    setSelectedCategory({
        ...
        <Form className="flex flex-col gap-x-6 mx-auto  px-24 lg:px-36 pb-36 xl:gap-x-10 bg-white-dirty bg-opacity-70 border-t border-gray border-opacity-40 pt-30">
          ...
          >
            <ImageField
              ...
              //  @ts-ignore
              imgSrc={product.FtpBannerPhotoFilePath}
              // @ts-ignore
              base64={base64}
              setBase64={setBase64}
            />
