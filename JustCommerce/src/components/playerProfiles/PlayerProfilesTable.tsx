...
const PlayerProfilesTable: React.FC<IUsersTableProps> = ({
  loading,
  containerRef,
  lastItemRef,
  playerProfiles,
  ...
  const rows = sortedArray.map((player: any) => ({
    ...
    cols: [
      player.Name,
      player.Birthdate.slice(6, 10),
      player.Region,
      `${+player.AcademyRelationCount + +player.TrainerRelationCount}`,
      <AssociatedStatusInfo isActive={!player.IsBanned} />,
      // <AssociatedStatusInfo isActive={player.Associated} />,
      <UserStatusInfo
        className="flex gap-x-2 px-8 md: -ml-4 md:pl-2/10 lg:pl-3/10 2xl:pl-4/10"
        isActive={player.IsActivated}
      />,
    ],
  }));
...
export default PlayerProfilesTable;
