...
import { ConditionTemplate, ConditionTemplateSubgroup } from 'types/conditionTypes';
    ...
    <>
      <thead className='border border-gray border-opacity-30 bg-gray bg-opacity-30 '>
        <tr>
          ...
          <th className='border border-gray border-opacity-30  '>{groupName}</th>
          <th></th>
          <th></th>
          <th></th>
          ...
          <th>
            <IsPercentSelect
              ...
              onChange={({ value }) => {
                setFieldValue(`conditionsGroups.${groupIndex}.isValuePercentage`, value);
      ...
      {!isCollapsed && (
        <tbody>
          {subgroups.map(({ conditionsRows, id }, subGroupIndex) => (
            <Subgroups
              key={subGroupIndex + id}
